...
@section('page_title', 'Reporte de Salidas')
@section('page_subtitle', 'Consulta de materiales entregados a técnicos')
    ...
    <div class="card">
        <form method="GET" action="{{ route('reportes.salidas') }}">
            ...
            <div style="display:flex; gap:10px; flex-wrap:wrap;">
                ...
                <a href="{{ route('reportes.salidas') }}" class="btn btn-secondary">Limpiar</a>
            </div>
        </form>
    </div>
        ...
        <table>
            ...
            <tbody>
                @forelse($detalles as $detalle)
                    <tr>
                        <td>{{ $detalle->salida->fecha->format('d/m/Y') }}</td>
                        <td>#{{ $detalle->salida->id }}</td>
                        <td>{{ $detalle->salida->tecnico?->nombre_completo }}</td>
                        <td>{{ $detalle->producto?->nombre }}</td>
                        <td>{{ number_format((float)$detalle->cantidad, 2) }}</td>
                        <td>{{ $detalle->salida->trabajo_referencia ?: '—' }}</td>
                        <td>{{ $detalle->salida->usuario?->name }}</td>
                        <td>{{ $detalle->salida->estado }}</td>
                    </tr>
                @empty
                    <tr>
